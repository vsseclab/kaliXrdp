...
**INSTRUCTIONS:  Open a NEW elevated command prompt window (admin rights are required to open firewall ports for RDP and SSH) then type/paste the following command:**
    ...
    PowerShell -executionpolicy bypass -command "wget https://github.com/vsseclab/kaliXrdp/raw/main/kaliXrdp.cmd -UseBasicParsing -OutFile kaliXrdp.cmd ; .\kaliXrdp.cmd"
...
The script will download the [LxRunOffline](https://github.com/DDoSolitary/LxRunOffline) distro manager to bootstrap the installation.  Install times will vary depending on system performance and the presence of antivirus software.  A fast system with broadband Internet can complete the install in under 10 minutes and most machines will complete within 20 minutes.  Expect it to take 30+ minutes if Windows Defender is running.  If you want to track progress logs are located in %TEMP%\kaliXrdp. 
...
**Configure kaliXrdp to start at boot (like a service, no console window)**
...
- Edit ```kaliXrdp.cmd```.  On line 2 you will see ```SET GITORG=vsseclab``` - Change ```vsseclab``` to the name of your own repository.
- Customize the script any way you prefer.
- Launch the script using your repository name:
 ```PowerShell -executionpolicy bypass -command "wget https://github.com/YOUR-ORG/kaliXrdp/raw/main/kaliXrdp.cmd -UseBasicParsing -OutFile kaliXrdp.cmd ; .\kaliXrdp.cmd"```
