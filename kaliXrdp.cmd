...
[Net.SecurityProtocolType]::Tls12; wget https://github.com/vsseclab/Pi-Hole-for-WSL1/raw/master/LxRunOffline-v3.5.0-33-gbdc6d7d-msvc.zip
...
-r '%TEMP%\kaliXrdp\LxRunOffline-v3.5.0-33-gbdc6d7d-msvc\*.exe' '%TEMP%'" > NUL
...
%GO% "wget -q https://dl.google.com/linux/direct/chrome-remote-desktop_current_amd64.deb -O /tmp/chrome-remote-desktop_current_amd64.deb ; DEBIAN_FRONTEND=noninteractive apt-fast -y install tilix atril engrampa seamonkey-mozilla-build nmap ncat ndiff /tmp/chrome-remote-desktop_current_amd64.deb /tmp/kaliXrdp/deb/zenmap_*.deb" > "%TEMP%\kaliXrdp\%TIME:~0,2%%TIME:~3,2%%TIME:~6,2% Extras [Seamonkey, Zenmap, CRD].log" 2>&1
...
%GO% "sed -i 's#SCHT#%SCHT%#g' /tmp/kaliXrdp/dist/usr/local/bin/restartwsl ; sed -i 's#DISTRO#%DISTRO%#g' /tmp/kaliXrdp/dist/usr/local/bin/restartwsl"

IF %LINDPI% GEQ 288 ( %GO% "sed -i 's/HISCALE/3/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/HISCALE/3/g' /tmp/kaliXrdp/dist/etc/profile.d/xWSL.sh" )
IF %LINDPI% GEQ 240 ( %GO% "sed -i 's/QQQ/120/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/III/60/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml ; sed -i 's/PPP/40/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml" )
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/HISCALE/2/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/HISCALE/2/g' /tmp/kaliXrdp/dist/etc/profile.d/xWSL.sh" )
IF %LINDPI% GEQ 192 ( %GO% "sed -i 's/Kali-Dark-HiDPI/Kali-Dark-xHiDPI/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml ; sed -i 's/QQQ/96/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/III/48/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml ; sed -i 's/PPP/32/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml" )
IF %LINDPI% LSS 192 ( %GO% "sed -i 's/HISCALE/1/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/HISCALE/1/g' /tmp/kaliXrdp/dist/etc/profile.d/xWSL.sh ; sed -i 's/QQQ/%LINDPI%/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xsettings.xml ; sed -i 's/III/%ICONS%/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-desktop.xml ; sed -i 's/PPP/%PANEL%/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfce4-panel.xml" )
IF %LINDPI% LSS 120 ( %GO% "sed -i 's/Kali-Dark-HiDPI/Kali-Dark/g' /tmp/kaliXrdp/dist/etc/skel/.config/xfce4/xfconf/xfce-perchannel-xml/xfwm4.xml" )

%GO% "sed -i 's/\\h/%DISTRO%/g' /tmp/kaliXrdp/dist/etc/skel/.bashrc"
...
%GO% "sed -i 's/WSLINSTANCENAME/%DISTRO%/g' /tmp/kaliXrdp/dist/usr/local/bin/initwsl"
...
%GO% "cp -Rp /tmp/kaliXrdp/dist/* / ; cp -Rp /tmp/kaliXrdp/dist/etc/skel/.* /root ; chmod +x /etc/init.d/xrdp ; update-rc.d -f xrdp defaults ; update-rc.d -f inetutils-syslogd enable S 2 3 4 5 ; update-rc.d -f ssh enable S 2 3 4 5 ; update-rc.d -f avahi-daemon enable S 2 3 4 5 ; apt-get clean ; cd /tmp" >NUL 2>&1
...
%GO% "sed -i 's/PLACEHOLDER/%XU%/g' /tmp/kaliXrdp/xWSL.rdp"
%GO% "sed -i 's/COMPY/LocalHost/g' /tmp/kaliXrdp/xWSL.rdp"
%GO% "sed -i 's/RDPPRT/%RDPPRT%/g' /tmp/kaliXrdp/xWSL.rdp"
%GO% "cp /tmp/kaliXrdp/xWSL.rdp ./xWSL._"
...
TYPE .tmp>.tmpsec.txt
COPY /y /b xWSL._+.tmpsec.txt "%DISTROFULL%\kaliXrdp (%XU%).rdp" > NUL
...
ECHO Set OW = GetObject(^"winmgmts:^" ^& ^"^{impersonationLevel^=impersonate^}!\\.\root\cimv2^") > "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO Set ST = OW.Get(^"Win32_ProcessStartup^") >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO Set OC = ST.SpawnInstance_ >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO OC.ShowWindow ^= 0 >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO Set OP = GetObject(^"winmgmts:root\cimv2:Win32_Process^") >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO WScript.Sleep 2000 >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO RT = OP.Create( ^"WSLCONFIG.EXE /t kali-linux^", null, OC, intProcessID) >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO WScript.Sleep 5000 >> "%LOCALAPPDATA%\kaliXrdp.vbs"
ECHO RT = OP.Create( ^"WSL.EXE ~ -u root -d kali-linux -e initwsl 2^", null, OC, intProcessID) >> "%LOCALAPPDATA%\kaliXrdp.vbs"
POWERSHELL -Command "Copy-Item '%DISTROFULL%\kaliXrdp (%XU%).rdp' ([Environment]::GetFolderPath('Desktop'))"
ECHO Building Scheduled Task...
%GO% "cp /tmp/kaliXrdp/xWSL.xml ."
...
POWERSHELL -C "$WAC = '%LOCALAPPDATA%\kaliXrdp.vbs' ; (Get-Content .\xWSL.xml).replace('QQQQ', $WAC) | Set-Content .\xWSL.xml"
...
ECHO:
ECHO:Installation of kaliXrdp (%DISTRO%) complete.
ECHO:Remote Desktop session will start shortly...
PING -n 6 LOCALHOST > NUL
START "Remote Desktop Connection" "MSTSC.EXE" "/V" "kaliXrdp (%XU%).rdp"
CD ..
ECHO:
:ENDSCRIPT
